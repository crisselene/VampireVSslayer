...
public class Game implements IPrintable {
	...
	public Game(Long seed, Level level) {
		...
		Vampiro.vampiros_restantes = level.getNumberOfVampires();
	}
	...
	private boolean userVictory() {
		return (Vampiro.vampiros_restantes == 0 && Vampiro.vampiros_tablero == 0);
	}
	...
	public String getInfo() {
		String info = ("Number of cycles: " + ciclos + "\n" +
						...
						"Remaining vampires: " + Vampiro.vampiros_restantes + "\n" +
						"Vampires on the board: " + Vampiro.vampiros_tablero + "\n");
	...
	private void creacionVampiros() {	
		if(Vampiro.vampiros_restantes > 0) crearVampiro();
		if(Vampiro.vampiros_restantes > 0 && !Dracula.draculaOnBoard) crearDracula();		
		if(Vampiro.vampiros_restantes > 0) crearExplosivo();	
	}

	private void crearExplosivo() {
		int columna = (level.getDimx() - 1);
		int fila = board.comprobarCrearVamp(columna, random.nextDouble() , random);
		if(fila != -1) {
	...
	public boolean tryAddVampire(int x, int y, String type) throws CommandExecuteException, CommandParseException {
		if (Vampiro.vampiros_restantes > 0) {
			if (board.dentroTablero(y, x)) {
				if (!board.buscarObjeto(x, y)) {
					if (type != null) {
						if (type.equals("d")) {
	...
	public String serialize() {
		serialize = "Cycles: " + ciclos + "\n" + 
					...
					"Level: " + level.name() + "\n" +
					"Remaining Vampires: " + Vampiro.vampiros_restantes + "\n" +
					"Vampires on Board: " + Vampiro.vampiros_tablero + "\n" + "\n" +
					"Game Object List: \n";
